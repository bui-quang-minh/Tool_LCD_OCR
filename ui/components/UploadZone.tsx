    ...
    <div className="space-y-4">
      ...
      >
        ...
        <UploadSimple
          size={28}
          weight="light"
          className={`mb-3 transition-colors ${dragging ? "text-zinc-300" : "text-zinc-500"}`}
        />

        <p className={`text-sm transition-colors ${dragging ? "text-zinc-200" : "text-zinc-400"}`}>
          {dragging ? "Drop to analyze" : "Drop image or click to browse"}
        </p>
        <p className="text-xs text-zinc-600 mt-1">JPG · PNG · BMP · WEBP</p>
      </div>

      {error && (
        <div className="flex items-start gap-3 p-4 bg-red-950/40 border border-red-900/60 rounded-lg">
          ...
          <div>
            ...
            <p className="text-red-400/60 text-xs mt-0.5 font-mono break-all">{error}</p>
          </div>
        </div>
      )}
    </div>
