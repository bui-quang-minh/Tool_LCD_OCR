"""
Fine-tune the LCD detection model (YOLO OBB) on a small dataset,
...
MODEL_NAME = os.environ.get("MLFLOW_DETECTION_MODEL_NAME", "lcd-ocr-detection")
BASE_MODEL = os.environ.get("BASE_DETECTION_MODEL", "/opt/mlops/lcd_dataset/base_detection.pt")
DATA_YAML = "/opt/mlops/lcd_dataset/detection.yaml"
...
def main():
    ...
    mlflow.set_experiment("lcd-ocr-detection-training")

    print(f"Loading base model: {BASE_MODEL}")
    model = YOLO(BASE_MODEL)

    with mlflow.start_run(run_name=f"train-detection-{int(time.time())}") as run:
        ...
        # ── Save best weights ───────────────────────────────────────────────────
        best_weights = "/tmp/lcd_runs/detection_finetune/weights/best.pt"
